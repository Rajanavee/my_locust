...
class Runner(object):
    ...
    def spawn_users(self, spawn_count, hatch_rate, wait=False,start_num = 0):
        ...
        def hatch(start_num = 0):
            ...
            while True:
                ...
                new_user = user_class(self.environment,start_num)
    ...
    def start(self, user_count, hatch_rate, wait=False,start_num = 0):
        """
        ...
        if self.state != STATE_INIT and self.state != STATE_STOPPED:
            ...
            if self.user_count > user_count:
                ...
                self.spawn_users(spawn_count=spawn_count, hatch_rate=hatch_rate,start_num = start_num )
            else:
                self.environment.events.hatch_complete.fire(user_count=self.user_count)
        else:
...
class LocalRunner(Runner):
    ...
    def start(self, user_count, hatch_rate, wait=False,start_num = 0):
        ...
        if self.hatching_greenlet:
            ...
            self.hatching_greenlet.kill(block=True)
        self.hatching_greenlet = self.greenlet.spawn(lambda: super(LocalRunner, self).start(user_count, hatch_rate, wait=wait,start_num = start_num))
